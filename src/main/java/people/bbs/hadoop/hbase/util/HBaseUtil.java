...
/**
 * HBase 工具类 Created by babylon on 2016/11/29.
 */
public class HBaseUtil {
	...
	// ConfigUtil.getInstance().getConfigVal("zkhost",
	// ConstantProperties.COMMON_PROP));
	 ...
	 * @throws IOException
	 */
	private static void createTable(String tableNameStr, String[] familyNames, byte[][] splitKeys) throws Exception {
	 ...
	 * @throws IOException
	 */
	private static void createTable(String tableNameStr, String[] familyNames) throws Exception {
		Connection conn = getConnection();
		HBaseAdmin admin = (HBaseAdmin) conn.getAdmin();
		TableName tableName = TableName.valueOf(tableNameStr);
		try {
			...
			logger.error("Table: {} create error! {}", tableName, e);
		} finally {
			admin.close();
			closeConnect(conn);
		}
	...
	public static void deleteTable(String tablenameStr) throws IOException {
		...
		try {
			if (!admin.tableExists(tableName)) {
				logger.warn("Table: {} is not exists!", tablenameStr);
				return;
			}
			admin.disableTable(tableName);
			admin.deleteTable(tableName);
			logger.info("Table: {} delete success!", tablenameStr);
		} finally {
			admin.close();
			closeConnect(conn);
		}
	...
	public static List<Result> list(String tablename) {
		Table table = getTable(tablename);
		List<Result> listResult = new ArrayList<Result>();
		try {
			ResultScanner rs = table.getScanner(new Scan());
			listResult = IteratorUtils.toList(rs.iterator());
			rs.close();
		} catch (IOException e) {
			logger.error("List failure !", e);
			return listResult;
		}
		return listResult;
	}
